...
const chaptersDir = path.join('C:/Users/diff/code/manhva-site/public/tokyo ghoul;re/', 'chapters');

// Количество глав
const totalChapters = 180;

// Шаблон страницы главы
  ...
  <header class="header">
    <nav>
      <div class="logo-container">
        ...
        <a href="/public/tokyo ghoul;re/index.html">Tokyo Ghoul:re</a>
      </div>
    </nav>
    <nav class="navbar">
      <a href="#">Закладки</a>
...
// Проверяем, существует ли папка с главами
if (!fs.existsSync(chaptersDir)) {
  console.error(`Папка 'chapters' не найдена по пути: ${chaptersDir}`);
  process.exit(1);
}

// Генерация HTML для каждой главы
for (let i = 1; i <= totalChapters; i++) {
  const chapterPath = path.join(chaptersDir, `${i}`);
  const imagesPath = path.join(chapterPath, 'images');

  console.log(`Обработка главы ${i}: ${chapterPath}`);

  // Проверяем, существует ли папка для главы
  if (!fs.existsSync(chapterPath)) {
    console.warn(`Пропущена глава ${i}: папка не найдена.`);
    continue;
  }
  ...
  if (images.length === 0) {
    console.warn(`Пропущена глава ${i}: изображения не найдены в ${imagesPath}`);
    continue;
  }
  ...
  fs.writeFileSync(path.join(chapterPath, 'index.html'), htmlContent, 'utf-8');
  console.log(`Глава ${i} создана с ${images.length} изображениями.`);
}